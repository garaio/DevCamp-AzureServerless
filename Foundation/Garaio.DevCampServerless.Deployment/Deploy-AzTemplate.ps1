...
Param(
    ...
    [string] $StorageContainerName = 'deployment',
...
if ($UploadArtifacts -Or $ArtifactsLocationParameter -ne $null) {
    ...
    if ($StorageAccountName -eq '') {
        $StorageAccountName = ($ResourceNamePrefix + 'sa' + $ResourceNameSuffix).Replace('-','')
    }
...
# Taken from: https://gist.github.com/rudfoss/31cae1e6062c9c49627586095e55a804
...
if(Test-Path $TemplateParametersFile){
    ...
    $TemplateParamObject = Get-AzTemplateParameters $TemplateParametersFile $TemplateParamObject
}

$TemplateArgs.Add('TemplateParameterObject', $TemplateParamObject)
